...
def gen_detailed_calcs(sec, Fy, Fu, cmp, gov, L, crane_cls, fat_cat, Lb, has_stiff, stiff_spa, 
    ...
    calcs.append({
        ...
        'calculations': [
            ...
            ('Allowable Stress Range',
             f"$F_{{SR}} = \\left( \\frac{{C_f}}{{n}} \\right)^{{0.333}} \\geq F_{{TH}}$",
             f"$F_{{SR}} = \\left( \\frac{{{fc['Cf']:.2e}}}{{{cycles}}} \\right)^{{0.333}} = {(fc['Cf']/cycles)**0.333:.2f}$ MPa"),
            ('', '', ''),
            ('**Stress Range Calculation:**', '', ''),
            ('Stress Range',
             f"$f_{{sr}} = \\frac{{M_{{range}}}}{{S_x}}$",
             f"(Based on moment range from crane loading cycles)"),
        ]
    })
    ...
    calcs.append({
        ...
        'calculations': [
            ...
            ('Deflection',
             f"Ratio = {ratios.get('Defl', 0):.3f}$",
             f"{'✓ OK' if ratios.get('Defl', 0) <= 1.0 else '✗ NG'}"),
            ('', '', ''),
            ('**Governing Check:**', '', ''),
            ('Maximum Ratio',
             f"",
             f"**{max(ratios.values()):.3f}** ({max(ratios, key=ratios.get)})"),
        ]
    })
...
def main():
    ...
    if st.session_state.get('run_design', False):
        ...
        with tabs[6]:
            ...
            # Also provide downloadable text version
            with st.expander("📄 Text Version (for download)"):
                calc_text = gen_calcs(sec, Fy, Fu, cmp, gov, beam_span, crane_cls, fat_cat, Lb, has_stiff, stiff_spa)
                st.text_area("Calculations", calc_text, height=300)
                st.download_button("📥 Download as TXT", calc_text, "calculations.txt")
